...
const ICONS = {
  ...
  heat:'<svg width="30" height="30" viewBox="0 0 30 30" fill="none" aria-hidden="true"><path d="M4 25h22" stroke="currentColor" stroke-width="1.3"/><path d="M9 20c-2-2.4 2-4.6 0-7s2-4.6 0-7M15 20c-2-2.4 2-4.6 0-7s2-4.6 0-7M21 20c-2-2.4 2-4.6 0-7s2-4.6 0-7" stroke="currentColor" stroke-width="1.3"/></svg>',
    ...
    '<button class="flip" type="button" aria-pressed="false" aria-label="Show the numbers for Eagle PPF '+esc(p.name)+' Series">'+
      '<span class="flip__inner">'+
        ...
        '<span class="flip__face flip__back">'+
          row("Thickness",spec("Thickness"))+row("Film",spec("Film"))+row("Warranty",spec("Warranty"))+
...
function viewHome(){
  ...
  <section class="section bg-graphite">
    <div class="wrap split">
      ...
      <div>${media(ASSETS.about,"Eagle PPF studio — paint protection film being installed in front of the Eagle PPF range",null,"1/1")}</div>
    </div>
  </section>
  ...
  <section class="section bg-graphite">
    <div class="wrap grid-3">
      <article class="card tilt" style="--acc:#E8B33A">
        <span class="card__shot card__shot--contain"><img src="${esc(DATA.precut.cardImage)}" alt="Eagle PPF Precut paint protection film" loading="lazy" /></span>
        <span class="card__glow"></span>
        <div class="card__body">
      ...
      <article class="card tilt" style="--acc:#8FB9D8">
        <span class="card__shot card__shot--contain"><img src="${esc(DATA.ceramic.image)}" alt="Eagle PPF Ceramic Coating kit" loading="lazy" /></span>
        <span class="card__glow"></span>
        <div class="card__body">
      ...
      <!-- Graphene ka apna box — click karne par Ceramic page ka Graphene hissa khulta hai -->
      <article class="card tilt" style="--acc:#9AA3AE">
        <span class="card__shot card__shot--contain"><img src="${esc(DATA.graphene.image)}" alt="Eagle PPF Graphene Coating being applied in the Eagle PPF studio" loading="lazy" /></span>
        <span class="card__glow"></span>
        <div class="card__body">
          <span class="card__cat">Graphene coating</span>
          <h3 class="h3">${esc(DATA.graphene.tagline)}.</h3>
          <p>Superior hardness, heat dissipation and anti-static protection — the next step beyond ceramic.</p>
          <div class="card__foot"><a class="link-arrow" href="#/ceramic?section=graphene">Graphene coating${ICONS.arrow}</a></div>
        </div>
      </article>
    </div>
  </section>
  ...
  <section class="section">
    <div class="wrap split">
      <div>
        ...
        <p class="lead" style="margin-top:16px">Answer three questions and we will point you at the grade that fits how you actually use the car — Crystal through to Platinum, precut, or a ceramic or graphene coating on top.</p>
...
function viewProducts(){
  ...
  <section class="section">
    <div class="wrap">
      ...
      <p class="lead">Five grades, one table. Thickness rises with the grade; so does the warranty behind it.</p></div>
      <div style="overflow-x:auto">
        <table class="spec" style="min-width:680px">
          <thead><tr><th scope="col" style="width:auto">Series</th><th scope="col">Thickness</th><th scope="col">Film</th><th scope="col">Warranty</th></tr></thead>
          <tbody>
            ${DATA.products.map(p=>{
              const spec = k => {
...
function viewPrecut(){
  ...
  <section class="section--tight" style="padding-top:0">
    <div class="wrap">
      ...
      ${c.image2 ? `<div class="frame sheen" style="margin-top:18px"><div class="frame__in">
        <img src="${esc(c.image2)}" alt="Eagle PPF Precut PPF packaging detail — box, film roll and unrolled sheet" loading="lazy" style="width:100%" />
      </div></div>` : ""}
    </div>
  </section>
...
function viewCeramic(){
  ...
  <section class="section section--tight">
    <div class="wrap">
      ...
      <div style="margin-top:26px"><a class="link-arrow" href="#/ceramic?section=graphene">Also available: Graphene coating${ICONS.arrow}</a></div>
    </div>
  </section>
  ...
  ${grapheneSection()}
  ${ctaBand("Book a ceramic or graphene coating.","Tell us the car and its condition and we will advise on preparation and finish.",true,false)}`;
}

/* ==========================================================================
   GRAPHENE COATING — alag page nahi; Ceramic page par Ceramic ke neeche aata hai.
   Link: #/ceramic?section=graphene (seedha is hisse par le aata hai)
   ========================================================================== */
function grapheneSection(){
  const g = DATA.graphene;
  return `
  <section class="section section-line" id="graphene" style="border-top:2px solid var(--red)">
    <div class="wrap">
      <p class="hero__label"><span class="meta">Graphene coating</span></p>
      <h2 class="h1" style="font-size:clamp(2.2rem,5vw,4.2rem)">${esc(g.title)}</h2>
      <p style="margin-top:18px;font-family:var(--font-display);font-size:clamp(1.1rem,2vw,1.5rem);letter-spacing:.16em;text-transform:uppercase;color:var(--red)">${esc(g.tagline)}</p>
      <p class="lead" style="margin-top:24px">${esc(g.intro)}</p>
      <div class="chips" style="margin-top:30px">${g.protects.map(x=>`<span class="chip">${esc(x)}</span>`).join("")}</div>
    </div>
  </section>
  ...
  <section class="section--tight" style="padding-top:0">
    <div class="wrap">
      <div class="frame sheen"><div class="frame__in">
        <img src="${esc(g.image)}" alt="Eagle PPF Graphene Coating being applied by hand in the Eagle PPF studio" loading="lazy" style="width:100%" />
      </div></div>
    </div>
  </section>
  <section class="section">
    <div class="wrap split">
      <div>
        <p class="meta">About this service</p>
        <h2 class="h2" style="margin-top:14px">Beyond ceramic</h2>
        <div style="margin-top:22px;display:grid;gap:18px">
          ${g.about.map(x=>`<p class="body-copy">${esc(x)}</p>`).join("")}
        </div>
      </div>
      <div>
        <p class="meta">Why you need this service</p>
        <h2 class="h2" style="margin-top:14px">Who it is for</h2>
        <div style="margin-top:22px;padding:24px;border:1px solid var(--line);border-left:2px solid var(--red);background:var(--panel);display:grid;gap:14px">
          ${g.why.map(x=>`<p class="body-copy" style="color:var(--white)">${esc(x)}</p>`).join("")}
        </div>
        <div class="btn-row" style="margin-top:28px"><a class="btn btn--red" href="#/contact">${DATA.cta.quote}</a></div>
      </div>
    </div>
  ...
  <section class="section bg-graphite">
    <div class="wrap">
      <div class="sec-head"><p class="meta">Key features</p><h2 class="h2">What graphene adds</h2></div>
      <div class="benefits">
        ${g.features.map(r=>`<div class="benefit tilt">${ICONS[r.icon]||ICONS.tick}<h3 class="h3">${esc(r.title)}</h3><p>${esc(r.text)}</p></div>`).join("")}
      </div>
    </div>
  </section>
  ...
  <section class="section">
    <div class="wrap split">
      <div>
        <p class="meta">Aftercare</p>
        <h2 class="h2" style="margin-top:14px">Care &amp; maintenance tips</h2>
        <ul class="tick-list" style="margin-top:22px">
          ${g.care.map(x=>`<li>${ICONS.tick}<span>${esc(x)}</span></li>`).join("")}
        </ul>
      </div>
      <div>
        <p class="meta">Cover</p>
        <h2 class="h2" style="margin-top:14px">Warranty</h2>
        <div style="margin-top:22px;padding:24px;border:1px solid var(--line);border-left:2px solid var(--red);background:var(--panel)">
          <p style="font-size:1.06rem;color:var(--white)">${esc(g.warranty)}</p>
        </div>
      </div>
    </div>
  </section>
    ...
    '<div class="stage" style="--acc:'+esc(p.accent)+'">'+
      ...
      '<div>'+
        ...
        '<div class="sel-summary" style="margin-top:22px">'+
          ...
          (spec("Film") ? '<span>Film <b>'+esc(spec("Film"))+'</b></span>' : '')+
        '</div>'+
...
const ASSIST = {
  ...
  daily: { bot:"City use is mostly parking scuffs, door edges and light scratches. Crystal (180 micron) covers that at the friendliest price, and Silver (190 micron) steps up to a five-year warranty if you want the cushion.",
    opts:[{label:"Crystal details",href:"#/products/crystal"},{label:"Silver details",href:"#/products/silver"},{label:"Start again",next:"start"}] },
  highway: { bot:"Highway running throws stones at the front end. Gold (200 micron) is the sensible step up — thicker film with a five-year warranty. Diamond (210 micron, eight-year warranty) if you keep cars long.",
    opts:[{label:"Gold details",href:"#/products/gold"},{label:"Diamond details",href:"#/products/diamond"},{label:"Start again",next:"start"}] },
  premium: { bot:"On a new or premium car the paint is the asset. Platinum (220 micron) is the top of the range, with a lifetime warranty. Diamond is the step below it.",
  ...
  gloss: { bot:"For gloss and easier washing, ceramic coating is the answer — a hydrophobic layer over the paint. It also sits happily on top of PPF. Graphene coating goes a step further, with added hardness, heat dissipation and anti-static protection.",
    opts:[{label:"Ceramic coating",href:"#/ceramic"},{label:"Graphene coating",href:"#/ceramic?section=graphene"},{label:"Start again",next:"start"}] }
};
...
function route(){
  ...
  else if(path === "/graphene")          { html = viewCeramic(); metaKey = "/ceramic"; }   // purana link -> Ceramic page ka Graphene hissa
  ...
  setDrawer(false);
  const jump = query.get("section") || (path === "/graphene" ? "graphene" : "");
  ...
  else if(!location.hash.includes("#/contact?")) window.scrollTo({top:0,behavior:"instant"});
  main.focus?.();
}
...
ld.textContent = JSON.stringify({
  ...
  foundingDate:"2015",
  areaServed:"IN",
  hasMap:"https://maps.app.goo.gl/Vyag3aNwXgYVz4wo7",
  url:"https://eagleppf.in/",
  logo:"https://eagleppf.in/android-chrome-512x512.png",
  image:"https://eagleppf.in/assets/img/products/platinum.jpg",
  address:{ "@type":"PostalAddress", streetAddress:"Block B, 76A, Dabri - Palam Rd, Vaishali, Vaishali Colony, Dashrath Puri",
