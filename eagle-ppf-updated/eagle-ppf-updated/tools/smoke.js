...
const routes = ["#/", "#/about", "#/products", "#/products/crystal", "#/products/silver", "#/products/gold", "#/products/diamond", "#/products/platinum", "#/precut", "#/ceramic", "#/graphene", "#/distributor", "#/contact", "#/contact?product=gold", "#/nonsense"];
...
const expect = [
  ...
  ["graphene link (inside ceramic)", /#\/ceramic\?section=graphene/],
...
// --- content audit (Sep 2026 update): ye cheezein site par kahin nahi honi chahiye ---
{
  const all = ["#/", "#/about", "#/products", "#/products/crystal", "#/products/silver", "#/products/gold",
               "#/products/diamond", "#/products/platinum", "#/precut", "#/ceramic", "#/graphene",
               "#/distributor", "#/contact", "#/nonsense"];
  const banned = [
    [/replacement|replace/i,              "'replacement' wording"],
    [/auto\s*re-?style/i,                 "Auto ReStyle brand name"],
    [/28\.6077065|google\.com\/maps\/place/, "old Google Maps link"],
    [/Dabri\u2013Palam|Dabri-Palam Road/, "old address format"]
  ];
  let bad = 0;
  for (const hash of all) {
    sandbox.location.hash = hash;
    vm.runInContext("route()", ctx);
    const html = main.innerHTML + bySelector("#footer").innerHTML + bySelector("#drawer").innerHTML;
    for (const [re, what] of banned) {
      if (re.test(html)) { console.error("\u274c " + hash + " still shows " + what); failures++; bad++; }
    }
  }
  if (!bad) console.log("\u2705 no replacement / Auto ReStyle / old address on any page");

  sandbox.location.hash = "#/ceramic";
  vm.runInContext("route()", ctx);
  const g = main.innerHTML;
  if (g.indexOf("Eagle PPF Ceramic Coating") > -1 && g.indexOf("Eagle PPF Ceramic Coating") < g.indexOf('id="graphene"'))
    console.log("\u2705 ceramic page: ceramic first, graphene below it");
  else { console.error("\u274c ceramic page order wrong / graphene section missing"); failures++; }
  const gExpect = [/Graphene Coating/, /future of paint protection/i, /At Eagle PPF, we apply premium graphene/,
                   /Water-spot resistance/, /pH-neutral/, /To be confirmed with our team/];
  if (gExpect.every(re => re.test(g))) console.log("\u2705 graphene page has all sections");
  else { console.error("\u274c graphene page is missing content"); failures++; }

  sandbox.location.hash = "#/contact";
  vm.runInContext("route()", ctx);
  const c = main.innerHTML;
  if (/maps\.app\.goo\.gl\/Vyag3aNwXgYVz4wo7/.test(c) && /Vaishali, Vaishali Colony/.test(c)) console.log("\u2705 contact uses the new address + Maps link");
  else { console.error("\u274c contact page address / Maps link not updated"); failures++; }
  if (!/#\/graphene/.test(bySelector("#nav").innerHTML + bySelector("#footer").innerHTML) && /Ceramic &amp; graphene coating|Ceramic & graphene coating/.test(bySelector("#footer").innerHTML))
    console.log("\u2705 no separate Graphene page link; footer says Ceramic & graphene coating");
  else { console.error("\u274c nav/footer still link a separate Graphene page"); failures++; }
}
